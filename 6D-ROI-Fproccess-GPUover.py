...
#-------------------------------
def create_experiment_folder(seed):
    base_dir="experiments"
    os.makedirs(base_dir, exist_ok=True)

    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    exp_name = f"SwOSMBiTM_Epoch{cfg.EPOCHS}_SEED{seed}_{timestamp}"
...
def main():
    ...
    import random
    import numpy as np
    '''from config_data import DataConfig as dcfg
    from config_model import ModelConfig as mcfg'''



    SEED= random.randint(0, 100)
    seed =set_seed(SEED)
    print(f"Using Seed in Main: {seed}")


        # Create experiment folder
    exp_dir = create_experiment_folder(seed=seed)
